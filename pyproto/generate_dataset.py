...
def generate_rotated_dataset():
    ...
    for class_name, item_img in items:
        ...
        for angle in range(0, 360, 8):  # 0, 45, 90, 135, 180, 225, 270, 315
            # 旋轉圖片
            rotated_item = rotate_image(item_img, angle)
            
            # 對 item 做高斯模糊 (在單獨item增強移除後，這步可以保留或調整)
            item_blur_kernel = random.choice(list(range(3, 19, 2))) 
            rotated_item[:, :, :3] = cv2.GaussianBlur(rotated_item[:, :, :3], (item_blur_kernel, item_blur_kernel), 0)
            
            # 生成單一物品的圖片 (背景 + item)
            # generate_single_item_image returns an RGBA image by default
            image, annotation_str = generate_single_item_image(rotated_item, class_name)
            
            # 解析YOLO標註以獲取像素座標和class_id
            # output_size is a global (width, height)
            class_id_str, x_c_norm, y_c_norm, w_norm, h_norm = annotation_str.split()
            class_id = int(class_id_str)
            
            x_c_px = float(x_c_norm) * output_size[0]
            y_c_px = float(y_c_norm) * output_size[1]
            w_px = float(w_norm) * output_size[0]
            h_px = float(h_norm) * output_size[1]
            
            x1_px = x_c_px - w_px / 2
            y1_px = y_c_px - h_px / 2
            x2_px = x_c_px + w_px / 2 # x1 + w_px
            y2_px = y_c_px + h_px / 2 # y1 + h_px

            single_box_data = [{'class_id': class_id, 'box': (x1_px, y1_px, x2_px, y2_px)}]

            # 對整張圖應用Shear和亮暗度調整，並更新邊界框
            image, updated_single_box_data_list = apply_full_image_augmentations(image, single_box_data)
            
            # 從更新後的邊界框數據重新生成YOLO標註
            updated_box = updated_single_box_data_list[0]['box']
            nx1, ny1, nx2, ny2 = updated_box
            
            new_w_px = nx2 - nx1
            new_h_px = ny2 - ny1
            # Ensure width and height are not negative due to clipping or extreme shear
            if new_w_px < 0: new_w_px = 0
            if new_h_px < 0: new_h_px = 0

            new_x_center_px = nx1 + new_w_px / 2
            new_y_center_px = ny1 + new_h_px / 2
            
            final_x_c_norm = new_x_center_px / output_size[0]
            final_y_c_norm = new_y_center_px / output_size[1]
            final_w_norm = new_w_px / output_size[0]
            final_h_norm = new_h_px / output_size[1]
            
            annotation_str = f"{class_id} {final_x_c_norm:.6f} {final_y_c_norm:.6f} {final_w_norm:.6f} {final_h_norm:.6f}"
...
def generate_image_two_stage(max_overlap=None, max_attempts=50):
    if max_overlap is None:
        max_overlap = random.uniform(0.1, 0.5)
...
total_images_to_generate = 16000
